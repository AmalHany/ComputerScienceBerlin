module.exports = function(app, route, express) {
  ...
  //this method is for checking out 
   app.post('/checkout', function(req, res){
    if(typeof req.user.cart!=='undefined'){
          var x=app.models.User.db.collection('users');
          //for every product in the cart of the session a transaction is created
...
var arry=[];
arry=req.user.cart;
// this is for emptying the cart of the user in the database
x.findAndModify(
  {_id: req.user._id}, // query
  [],  // sort order
  {$set: {cart: arry}}, // replacement, replaces only the field "cart"
  {}, // options
  function(err, object) {
      if (err){
...
// making sure the cart is empty by returning all products corresponding to the cart in the user database which is empty
Product.find({
